...
# Conversation history for each client
...
# Function to fetch stock exchange trends
def fetch_stock_exchange_trend(exchange):
    ...
    url = f"{BASE_URL}/v1/open-close/{exchange}/2024-12-01?apiKey={polygon_api_key}"
    try:
...
def fetch_stock_trends(ticker):
    ...
    start_date = "2024-01-01"  # Adjust dynamically if needed
    url = f"{BASE_URL}/v2/aggs/ticker/{ticker}/range/1/day/{start_date}/{today}?apiKey={polygon_api_key}"
    ...
    try:
        ...
        if not data.get("results"):
            return {"error": f"No trend data found for {ticker} in the specified period."}
...
def fetch_stock_exchange_trend(exchange):
    url = f"{BASE_URL}/v2/aggs/grouped/locale/us/market/{exchange}?apiKey={polygon_api_key}"
    print("Exchange",exchange)
    # url = f"{BASE_URL}/v1/open-close/{exchange}/2023-01-09?apiKey={polygon_api_key}"
    try:
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()
...
# Function to fetch historical stock trends
def fetch_stock_trends(ticker):
...
def format_stock_trends(trends, ticker):
    """
    Format stock trends into a user-friendly response.
    """
    ...
    try:
        ...
        for trend in trends[:5]:  # Limit to 5 recent trends
...
def fetch_industry_trends(industry):
    """
    Fetch trends for a specific industry.
    """
    print("industry : ",industry)
    # url = f"{BASE_URL}/v3/reference/market/industry/{industry}?apiKey={polygon_api_key}"
...
def format_company_details(details):
    """
    Format the company details into a chatbot-friendly response.
    """
    try:
...
def format_stock_exchange_trend(data, exchange):
    """
    Format stock exchange trend data into a user-friendly response.
    """
    if not data or ("error" in data):
    ...
    try:
        ...
        for trend in data.get('results', [])[:5]:  # Limit to 5 recent trends
...
def format_industry_trends(data, industry):
    """
    Format industry trends data into a user-friendly response.
    """
    if not data or ("error" in data):
    ...
    try:
        ...
        for company in data.get('results', [])[:5]:  # Limit to 5 companies
...
def get_openai_response(user_query):
    """ Generate a GPT response and call a relevant function based on the type of query. """
...
def get_gpt_response(previous_response):
    ...
    **Response Structure:**
    ...
    messages = [
        {
            "role": "system",
            "content": """ 
            You are an advanced conversational assistant designed to refine and enhance responses based on previous outputs. 
    ...
    try:
        ...
        print(f'Error in GPT Refinement: {e}')
        return {'error': f'Failed to refine response: {str(e)}'}

async def process_message_with_history(message, conversation_history):
    """ Process user input with conversation history and generate a response. """
    ...
    try:
        ...
        # print(" initial response of GPT : ", initial_response)
        
        # Check if initial_response is a string (chatbot response)
        if isinstance(initial_response, str):
            response = get_gpt_response(initial_response)
        else:
            # If it's not a string, process it with get_gpt_response
            response = get_gpt_response(initial_response)
        ...
        # print("Conversation Hostory of chat :",conversation_history)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Process the message and include history
            response = await process_message_with_history(message, conversation_history)
...
#     """
#     Handle incoming WebSocket connections and messages with conversation history.
#     """
#     conversation_history = []
...
#         except Exception as e:
#             error_response = json.dumps({"response": f"Internal server error: {e}"})
